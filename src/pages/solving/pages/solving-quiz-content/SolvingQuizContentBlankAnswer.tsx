...
const SolvingQuizContentBlankAnswer = ({
	...
	const blankGroups = questionInfo.answers.reduce((groups: any, answer: any) => {
	...
	const blankAnswers = Object.keys(blankGroups).map((number) => ({
		...
		value: userAnswers && userAnswers.find((ans: any) => ans.number === parseInt(number))?.answer || "",
	}));
	...
	const handleAnswerChange = (index: number, value: string) => {
		...
		const existingIndex = newAnswers.findIndex((ans: any) => ans.number === blankNumber);
		if (existingIndex >= 0) {
