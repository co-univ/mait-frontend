import { ToastContainer, toast, Bounce, type ToastOptions } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import { CircleAlert, TriangleAlert, CircleCheck, Info, X } from 'lucide-react';
...
const ICONS: Record<ToastType, React.ReactNode> = {
  ...
  warning: <TriangleAlert size={16} className={SHARED_COLOR_CLASSES.warning.text} />,
  success: <CircleCheck size={16} className={SHARED_COLOR_CLASSES.success.text} />,
 ...
 * 
 */
const CloseButton = ({ closeToast, type }: { closeToast?: (e: React.MouseEvent<HTMLElement>) => void; type: ToastType }) => {
  const colorClass = SHARED_COLOR_CLASSES[type].text;
  return (
    <button type="button" onClick={closeToast} className="p-[4px] self-start">
...
const toastWithMessage = (type: ToastType, message: string) => {
  ...
  const ToastBody = ({ closeToast }: { closeToast?: (e: React.MouseEvent<HTMLElement>) => void }) => (
    <div className="font-paperlogy flex items-center justify-between w-full h-full px-[1rem] py-[0.75rem]">
