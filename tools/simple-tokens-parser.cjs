...
function readTokenFiles() {
	...
	const metadata = JSON.parse(
		fs.readFileSync(path.join(tokensDir, "$metadata.json"), "utf8"),
	);
...
function resolveTokenReference(
	value,
	tokenSets,
	defaultTokenSetName = "Token/Mode 1",
) {
	if (
		typeof value === "string" &&
		value.startsWith("{") &&
			...
			case "typography":
				...
				if (typeof value === "object") {
					...
					if (value.fontFamily) {
						...
						if (fontToken) {
							typographyStyle.fontFamily = Array.isArray(fontToken.value)
								? fontToken.value
								: [fontToken.value];
						}
					}
					...
					if (value.fontSize) {
						...
						if (sizeToken) {
							const size =
								typeof sizeToken.value === "number"
									? `${sizeToken.value}px`
									: typeof sizeToken.value === "string" &&
											sizeToken.value.includes("rem")
										? `${parseFloat(sizeToken.value) * 16}px`
										: typeof sizeToken.value === "string" &&
												sizeToken.value.includes("px")
											? sizeToken.value
											: `${sizeToken.value}px`;
		...
		if (semanticTokens[category]) {
			Object.entries(semanticTokens[category]).forEach(([key, token]) => {
				if (token.value && token.type === "dimension") {
					...
					const value = resolveTokenReference(token.value, tokenSets);
...
if (theme.fontSize) {
	console.log(
		"Font sizes:",
		Object.keys(theme.fontSize).slice(0, 5).join(", "),
	);
}
if (theme.spacing) {
